#!/usr/bin/env python3
"""
ViT Attention Map Visualizer
============================
Loads RGB images from a directory, runs them through a pretrained
vit_base_patch16_224 model, extracts attention weights from every
Attention block via forward hooks, and saves one figure per block
per image.
...
Input images   : any size (512x512 recommended; resized to 224x224)
...
Model          : vit_base_patch16_224  (pretrained ImageNet-1K)
Output layout  :
...
class AttentionStore:
    """
    Attaches forward hooks to the attn_drop layer inside every Attention block.
    In eval mode attn_drop is identity, so its output is the post-softmax
    attention weights.

    Each record contains:
        key        : module path  e.g. "blocks.3.attn"
        block      : block index within the model (0-11 for ViT-Base)
        global_idx : sequential 0-based index across all blocks
        attn       : Tensor (B, num_heads, N, N)   N = NUM_PATCHES + 1
    """
    def __init__(self):
        self.records  = []
        self._handles = []
...
def reconstruct_spatial_map(attn, num_patches, grid_size):
    """
    Collapse (B, num_heads, N, N) into a normalised (grid_size, grid_size) map.
    N = num_patches + 1  (first token is CLS).

    1. Average over batch                -> (num_heads, N, N)
    2. Average over heads                -> (N, N)
    3. Average over keys (axis=-1)       -> (N,)   per-token score
    4. Drop CLS token (index 0)          -> (num_patches,)
    5. Reshape into spatial grid         -> (grid_size, grid_size)
    6. Min-max normalise                 -> [0, 1]
    """
    attn_mean    = attn.mean(dim=0).mean(dim=0)   # (N, N)
    token_scores = attn_mean.mean(dim=-1)          # (N,)
    patch_scores = token_scores[1:]                # drop CLS -> (num_patches,)
    spatial      = patch_scores.reshape(grid_size, grid_size).numpy()
...
def run(input_dir, output_dir,
    ...
    for img_path in image_paths:
        ...
        for rec in store.records:
            ...
            # ── predicted attention ────────────────────────────────────
            smap    = reconstruct_spatial_map(rec["attn"], NUM_PATCHES, GRID_SIZE)
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Visualise ViT per-block attention maps. "
